...
import 'package:moneyspace/sobre/sobre_page.dart';
...
class _HomeState extends State<Home> {
  ...
  _setSaldo() {
    ...
    if (_listfinance["carteira"].length > 0) {
      if (_listfinance["carteira"].length == 1 &&
          _listfinance["carteira"]["$ano"] != null) {
        for (l = 0; l < _listfinance["carteira"]["$ano"]["$mes"].length; l++) {
          if (_listfinance["carteira"]["$ano"]["$mes"][l]["gastos valor"] !=
              null) {
            calcgastosatual = UtilBrasilFields.converterMoedaParaDouble(
                    _listfinance["carteira"]["$ano"]["$mes"][l]
                                ["gastos valor"] !=
                            null
                        ? _listfinance["carteira"]["$ano"]["$mes"][l]
                            ["gastos valor"]
  ...
  _setSaldoTotGastos() {
    ...
    if (_listfinance["carteira"].length > 0) {
      for (l = 0; l < _listfinance["carteira"]["$ano"]["$mes"].length; l++) {
        if (_listfinance["carteira"]["$ano"]["$mes"][l]["gastos valor"] !=
            null) {
          calcgastos = UtilBrasilFields.converterMoedaParaDouble(
  ...
  _setSaldoReceita() {
    ...
    if (qtdAno > 0) {
      int menosMes =
          _convertMes[0][mes] - 1 != 0 ? _convertMes[0][mes] - 1 : 12;
      bool verificAno = false;
      bool verificMes = false;
      while (verificAno == false || verificMes == false) {
        if (_listfinance["carteira"]["$initAno"] != null) {
          for (l = 0;
              l < _listfinance["carteira"]["$initAno"][_mes[0][initMes]].length;
              l++) {
            if (_listfinance["carteira"]["$initAno"][_mes[0][initMes]][l]
                    ["gastos valor"] !=
                null) {
              calcgastospassado = UtilBrasilFields.converterMoedaParaDouble(
                      _listfinance["carteira"]["$initAno"][_mes[0][initMes]][l]
                                  ["gastos valor"] !=
                              null
                          ? _listfinance["carteira"]["$initAno"]
                              [_mes[0][initMes]][l]["gastos valor"]
                          : 0.0) +
                  calcgastospassado;
              //print("Esse é o gasto do Mês $initMes do Ano $initAno:  $calcgastospassado");
            } else {
              calcreceitaspassado = UtilBrasilFields.converterMoedaParaDouble(
                      _listfinance["carteira"]["$initAno"][_mes[0][initMes]][l]
                                  ["receita valor"] !=
                              null
                          ? _listfinance["carteira"]["$initAno"]
                              [_mes[0][initMes]][l]["receita valor"]
                          : 0.0) +
                  calcreceitaspassado;
    ...
    if (idCarteira == 1 && _listfinance["carteira"]["$ano"] != null) {
      for (l = 0; l < _listfinance["carteira"]["$ano"]["$mes"].length; l++) {
        if (_listfinance["carteira"]["$ano"]["$mes"][l]["receita valor"] !=
            null) {
          calcreceitasatual = UtilBrasilFields.converterMoedaParaDouble(
                  _listfinance["carteira"]["$ano"]["$mes"][l]
                              ["receita valor"] !=
                          null
                      ? _listfinance["carteira"]["$ano"]["$mes"][l]
        ...
        if (_listfinance["carteira"].length > 0) {
          if (_listfinance["carteira"]["$ano"] != null) {
            ...
            if (recContact["receita valor"] != null) {
              ...
              double percent = _setSaldoReceita() *
                  _listmetas[0]["metas mes"]["$mes"][0]
                      [convertList[0][recContact["tipo de gastos"]]];
              double gastoTot = _setGastoTotal(recContact["tipo de gastos"]) +
                  UtilBrasilFields.converterMoedaParaDouble(
                      recContact["gastos valor"]);
              ...
              if (percent >= gastoTot) {
                ...
                setState(() {
                  _iforText = "É necessário que você aumente sua receita ou reduza os gastos. Não há  saldo disponível";
                  _typeIforText = "!ALERT!\n";
                });
              }
            }
            ...
            setState(() {
              _listfinance["carteira"]["${recContact["ano"]}"] = {
                "Janeiro": [],
                "Fevereiro": [],
                "Março": [],
                "Abril": [],
        ...
        setState(() {
          if (tipo == 1) {
            result = calcgastostipo /
                (_setSaldoReceita() *
                    _listmetas[0]["metas mes"]["$mes"][0]["gastos essenciais"]);
          } else if (tipo == 2) {
            result = calcgastostipo /
                (_setSaldoReceita() *
                    _listmetas[0]["metas mes"]["$mes"][0]
            ...
            result = calcgastostipo /
                (_setSaldoReceita() *
                    _listmetas[0]["metas mes"]["$mes"][0]["investimentos"]);
          }
        ...
        child: Column(
          children: [
            UserAccountsDrawerHeader(
                ...
                accountEmail: Text('Bora organizar as finanças?')),
            ListTile(
            ...
            ListTile(
              ...
              subtitle: Text('Definir % orçamento'),
              onTap: () {
                if (_setSaldoTotGastos() > 0 ||
                    mes != _mes[0][DateTime.now().month]) {
                  setState(() {
                    _iforText =
                        "Para mudar as porcentagens das metas remova as despesas!";
                    _typeIforText = "!ALERT!\n";
                  });
                } else {
            ...
            ListTile(
              ...
              title: Text('Sobre'),
              subtitle: Text('Saiba como funciona o app'),
              onTap: () {
                Navigator.pushReplacement(
                  context,
                  MaterialPageRoute(
              ...
              children: [
                Expanded(
                    ...
                    child: ChartWidget(
                      ...
                      varPercent: (_listmetas[0]["metas mes"]["$mes"][0]
                                  ["gastos não essenciais"] *
                              100)
                          .ceil(),
                      label: "vermelho",
  ...
  Widget _gastosCard(BuildContext context, int index) {
    return Dismissible(
                    ...
                    Container(
                      ...
                      Padding(
                        ...
                        child: Column(
                          ...
                          children: [
                            Text(
                              _listfinance["carteira"]["$ano"]["$mes"][index]
                                          ["gastos descrição"] !=
                                      null
      ...
      onDismissed: (direction) {
        setState(() {
          _lastRemoved =
              Map.from(_listfinance["carteira"]["$ano"]["$mes"][index]);
          _lastRemovedPos = index;
          dynamic anoOrigi = DateTime.now().year;
          dynamic mesOrigi = DateTime.now().month;

          final verific = ("$anoOrigi" == "$ano") &&
              ("$mesOrigi" == _convertMes[0][mes].toString());
          print(verific);

          if (_lastRemoved["receita valor"] != null) {
            final rec = _setSaldoReceita();
            final receitaTotal = (rec) -
                (UtilBrasilFields.converterMoedaParaDouble(
                            _lastRemoved["receita valor"]) != null ? UtilBrasilFields.converterMoedaParaDouble(
                        _lastRemoved["receita valor"])
                    : UtilBrasilFields.converterMoedaParaDouble(
                                _lastRemoved["gastos valor"]) != null ? UtilBrasilFields.converterMoedaParaDouble(
                            _lastRemoved["gastos valor"])
                        : 0);
            ...
            if (result >= 0 && resp == true && verific == true) {
              ...
              setState(() {
                _iforText =
                    "Ação não realizada, por favor exclua um gasto antes";
                _typeIforText = "!ALERT!\n";
              });
            }
  ...
  Image selectImage(int index) {
    if (_listfinance["carteira"]["$ano"]["$mes"][index]["receita descrição"] !=
        null) {
